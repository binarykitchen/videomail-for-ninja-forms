...
stage_and_commit_changes () {
  ...
  if [ -d "$ASSETS_DIR" ]; then
	...
	# mh: added -r because of https://github.com/rayrutjes/wp-plugin-git-svn-sync/issues/15#issuecomment-1566308240

  find . -type f -name "*.png" \
    | awk '{print $0 "@"}' \
    | xargs -r svn propset --quiet --force svn:mime-type image/png
